...
ADDR_BITS_TO_RESET = 5; #2xenderecos possiveis de hash 7~bits

for i in range(2**ADDR_BITS_TO_RESET):
...
#tempo de reset = escrever zeros en 2**N bits
# + ler zeros en 2**N bits mais delay de pipeline com folga
#simReg.regDelay(2*8*2**ADDR_BITS_TO_RESET) #espera resetar a memoria
#simReg.regDelay(8*10) #espera tempo extra
